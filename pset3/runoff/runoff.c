...
bool vote(int voter, int rank, string name)
{
    ...
    for (int c = 0; c < candidate_count; c++)
    {
        ...
        //Check if Candidate exists, then increment their votes
        if (strcmp(candidates[c].name, name) == 0)
        {
            preferences[voter][rank] = c;
            //candidates[c].votes++;
            //printf("Current voter %i ; rank %i ; name %s\n", voter, rank, name);
            //printf("Candidate = n.%i %s \n", preferences[voter][rank], candidates[c].name);
